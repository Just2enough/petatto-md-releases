...
## Microsoft Store 版（MSIX）について

> ✅ **Microsoft Store で公開中です** → **[Microsoft Store で入手する](https://apps.microsoft.com/detail/9N77B4HPVRBB)**

Petatto.md は GitHub 配布の MSI 版に加え、Microsoft Store からの **MSIX 版**を提供しています。Store 版には次の特徴があります。

- **SmartScreen 警告が出ません**（Microsoft がパッケージに署名するため。MSI 版で出る「不明な発行元」表示が不要）。
- **インストール／アンインストールがクリーン**（Store 経由で導入・削除）。

### ⚠️ MSI 版と Store 版の併用は非推奨

どちらか一方をお使いください。両方を入れると状態データ（DB）の扱いが分かれます。

- **両方を新規に入れた場合**：付箋の位置・色などの状態は各バージョンで**別々に管理**され、互いに同期されません。
- **既に MSI 版があるマシンに Store 版を入れた場合**：両者が**同じ状態データ（DB）を共有**します。同時に起動すると同じ付箋が二重に表示されるなど競合し得ます。

### MSI 版から Store 版へ移行する場合

- **自動起動（Windows ログイン時の起動）の設定は引き継がれません**。Store 版で改めて、メインウィンドウの「**メンテナンス**」カードにある「Windows 起動時に自動起動する」を ON にしてください（Store 版はログイン自動起動を Windows の StartupTask で管理する方式のため、MSI 版の設定はそのまま移りません）。
- 状態データは上記のとおり、MSI 版が残っているマシンでは DB を共有します。MSI 版を残す必要がなければ、先に MSI 版をアンインストールしてから Store 版を入れると状態が一本化されます。
...
タスクトレイの Petatto.md アイコンを **左クリック**すると、表示中の全付箋が一時的に最前面に持ち上がります（他のウィンドウの裏に回った付箋を、メニューを開かず 1 アクションで前面に呼び出す近道。下記メニューの「全付箋を最前面に表示」と同じ動作）。
...
- **付箋が画面外に行ってしまった**: トレイの「全付箋を中央に集める」または「全付箋を最前面に表示」で回収（トレイアイコンの**左クリック**でも前面化できます）
