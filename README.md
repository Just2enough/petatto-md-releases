...
### 方法 3: md を新規作成してそのまま付箋にする

「新しいメモを今すぐデスクトップに貼りたい」ときは、md の作成から Petatto.md だけで完結できます。

タスクトレイの Petatto.md アイコンを右クリック → 「**新規md作成...**」でポップアップが開きます。

![新規md作成ポップアップ](docs/screenshots/new_md.png)

1. **ファイル名**を入力します（`.md` は自動で付きます）
2. **保存先**は既定で vault のルートです。「**変更...**」で vault 内の別フォルダを選べます（保存先を記憶する設定はなく、次に開いたときは vault のルートに戻ります）
3. 「**作成**」を押すと `petatto-md: true` 付きの md が生成され、すぐデスクトップに付箋として現れます

同じ名前の md が既にある場合は作成せずエラーになります（既存のファイルは変更しません）。「**キャンセル**」または **Esc** でいつでも閉じられます。

### 付箋化の解除

付箋ヘッダの「**×**」ボタンを押すと付箋化が解除され、付箋が消えます（**md ファイル自体は残ります**）。

解除したときに frontmatter をどう扱うかは、メインウィンドウの「**付箋**」カードにある「**×ボタンで付箋を解除するとき**」で選べます。

| 選択 | 解除後の md |
|---|---|
| **`petatto-md:` の frontmatter を md ファイルから削除する**（既定） | キーごと削除されます（他にキーが無ければ frontmatter ブロックごと消えます） |
| **`petatto-md: false` を書き残す** | キーは残り、値だけが `false` になります。frontmatter の他のキーやコメントはそのまま保たれます |

書き残す方を選ぶと、Obsidian 側で `petatto-md: false` を検索して「**前に付箋にしていたノート**」を辿れます。設定は次回以降の解除にも引き継がれます。

どちらの場合も、トレイの「**新規付箋化...**」から選び直せば再び付箋になります（`petatto-md: false` のファイルも選べます）。

> ⚠️ 付箋の位置・サイズ・色は付箋化を解除した時点で破棄されます。再付箋化しても元には戻りません（既定値から始まります）。

### フォルダを監視対象から除外する（`.petatto-ignore`）

Templates フォルダのように「`petatto-md: true` を含む md が入っているが付箋にはしたくない」場所は、そのフォルダを丸ごと監視対象から外せます。

除外したいフォルダの中に、**拡張子のない空ファイル `.petatto-ignore`** を 1 つ置いてください。そのフォルダ配下（サブフォルダも含め何段でも）の md は、`petatto-md: true` が付いていてもスキャンされず、付箋になりません。

- **作り方**: Obsidian はドット始まりのファイルを作れないため、**エクスプローラ**など OS 側の操作で作成します（例: 対象フォルダ内で右クリック → 新規作成 → テキスト ドキュメント → 名前を `.petatto-ignore` に変え、拡張子を消す）。中身は空で構いません。
- **正しい手順**: **`.petatto-ignore` を置いてから**、対象ノートの `petatto-md` を編集します。逆順にすると `true` にした一瞬だけ付箋が現れますが、**アプリを起動し直せば**出てこなくなります（md はそのまま。付箋ヘッダの「×」は `petatto-md` 自体を書き換えて付箋化を解除する操作なので、ここでは使わないでください）。
- **すでに出ている付箋**: 後から `.petatto-ignore` を置いても、その時点で開いている付箋は閉じません。**次にアプリを起動したとき**から出てこなくなります。
- **範囲**: vault のルート直下に置くと vault 全体が除外されます。除外中のフォルダのファイルは、トレイの「**新規付箋化...**」でも選べません（「除外設定されているフォルダのファイルは付箋化できません」と表示されます）。「**新規md作成...**」の保存先にも選べません（「このフォルダは監視の対象外です」と表示されます）。
- **元に戻す**: `.petatto-ignore` を削除すると、次回の起動（または配下ファイルの次の更新）から自動的に対象へ戻ります。

> Obsidian のテンプレート機能を使っている場合は、`.petatto-ignore` を置くのが確実です。フォルダ名を `.Templates` のようにドット始まりへ変える回避策は**おすすめしません**（Obsidian 自身もドット始まりフォルダを無視するため、テンプレートとして機能しなくなります）。
...
- **新規md作成...**: ファイル名を入力して vault 内に新しい md を作り、そのまま付箋化
